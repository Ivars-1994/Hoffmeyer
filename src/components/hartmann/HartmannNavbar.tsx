...
const HartmannNavbar = ({ brandName = 'KELLERMANN' }: HartmannNavbarProps) => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  return (
    <nav className="bg-[#064A2A] py-4 px-4 md:px-8 sticky top-0 z-50">
      ...
      {isMenuOpen && (
        <div className="md:hidden mt-4 pt-4 border-t border-[#006622]">
          <div className="flex flex-col gap-4">
            ...
            >
              <Phone size={18} />
              <span className="font-semibold">01579 2492770</span>
            </a>
          </div>
        </div>
      )}
